...
export const documentQueueService = {
  ...
  async processQueue() {
    ...
    try {
      ...
      console.error('Error processing document submission queue:', error);
    } finally {
  ...
  async processItem(id: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to record submission status to database:', dbError);
      }
      ...
      try {
        ...
        console.error('Failed to record submission failure to database:', dbError);
      }
  ...
  async recordStatus(item: SubmissionQueueItem): Promise<void> {
    try {
      const { error } = await supabase
        .from('document_submission_queue')
        .upsert({
          id: item.id,
          user_id: item.userId,
      ...
      console.error('Failed to record submission status:', error);
      throw error;
    }
  },
